                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px; background:#ffffff; border:1px solid #e5e7eb; border-radius:14px; overflow:hidden;">
                    <tr>
                        <td style="padding:24px 26px; background:linear-gradient(90deg, #2563eb 0%, #1d4ed8 100%); color:#ffffff;">
                            ...
                            <div style="font-size:16px; margin-top:10px; line-height:1.5; opacity:.95;">{{ $messageLine }}</div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding:22px 26px 10px 26px;">
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="font-size:15px; color:#374151; line-height:1.55;">
                                <tr>
                                    <td style="padding:5px 0;"><strong>Order #:</strong> {{ $order->id }}</td>
                                    <td style="padding:5px 0; text-align:right;">
                                        ...
                                        @else
                                            <span style="display:inline-block; padding:3px 8px; border-radius:999px; font-size:12px; font-weight:700; background:#E5E7EB; color:#374151;">{{ ucfirst($order->status) }}</span>
                                        @endif
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding:5px 0;"><strong>Customer:</strong> {{ $order->user->name }}</td>
                                    <td style="padding:5px 0; text-align:right;"><strong>Email:</strong> {{ $order->user->email }}</td>
                                </tr>
                                @if($order->payment)
                                    <tr>
                                        <td style="padding:5px 0;"><strong>Payment:</strong> {{ $paymentLabels[$order->payment->method] ?? $order->payment->method }}</td>
                                        <td style="padding:5px 0; text-align:right;"><strong>Payment Status:</strong> {{ ucfirst($order->payment->status) }}</td>
                                    </tr>
                                @endif
                            </table>
                        </td>
                                        ...
                                        <tr>
                                            <td style="padding:11px 0; border-bottom:1px solid #f3f4f6;">
                                                ...
                                                @if($item->variant->variant_name)
                                                    <span style="color:#6b7280; font-size:13px;">({{ $item->variant->variant_name }})</span>
                                                @endif
                                            </td>
                                            <td style="padding:11px 0; border-bottom:1px solid #f3f4f6; text-align:center;">{{ $item->quantity }}</td>
                                            <td style="padding:11px 0; border-bottom:1px solid #f3f4f6; text-align:right;">₱{{ number_format((float) ($item->unit_price * $item->quantity), 2) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
