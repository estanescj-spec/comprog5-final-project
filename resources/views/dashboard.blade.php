    ...
    <div class="pt-4 pb-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            <section class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-blue-100 w-screen max-w-none relative left-1/2 -translate-x-1/2">
                ...
                <div class="relative">
                    @if ($coverImage)
                        <img src="{{ $coverImage }}" alt="Featured" class="block w-full h-auto">
                    @else
                    ...
                    <div class="absolute inset-0 grid grid-cols-1 lg:grid-cols-2">
                        ...
                        <div class="flex items-center justify-end p-6 sm:p-8">
                            <div class="text-black max-w-xl text-right mr-8 sm:mr-12 lg:mr-20" style="text-align: left !important;">
                                <h1 class="text-2xl sm:text-3xl font-bold mb-2 text-black" style="text-align: left !important;">{{ $bannerTitle }}</h1>
                                <p class="text-sm sm:text-base text-black/80" style="text-align: left !important;">{{ $bannerSubtitle }}</p>
                                @auth
                                    <a href="{{ route('products.index') }}" class="inline-flex mt-4 items-center bg-blue-500 text-white font-medium rounded-full hover:bg-blue-500" style="padding: 1.3rem 2.1rem !important; font-size: 10px !important; line-height: 1 !important;">SHOP NOW</a>
                                @else
                                    <a href="{{ route('login') }}" class="inline-flex mt-4 items-center bg-blue-500 text-white font-medium rounded-full hover:bg-blue-500" style="padding: 1.4rem 2.1rem !important; font-size: 10px !important; line-height: 1 !important;">SHOP NOW</a>
                                @endauth
                            </div>
                        </div>
                    </div>
                ...
                @auth
                    @if (auth()->user()->role === 'admin')
                        <div class="border-t border-blue-100 p-4 sm:p-6 bg-blue-50/40">
                            <form method="POST" action="{{ route('admin.home.featured-image.update') }}" enctype="multipart/form-data" class="flex flex-col sm:flex-row sm:items-center gap-3">
                                @csrf
                                @method('PATCH')
            ...
            <section class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-blue-100 p-6">
                ...
                <div class="pb-2">
                    ...
                    @foreach ($newReleases->take(4) as $product)
                        @php
                    ...
                    <div class="flex justify-end mt-2">
                        <a href="{{ route('products.index') }}" class="inline-flex items-center gap-1 text-xs text-blue-700 hover:text-blue-800">
                            View all products
