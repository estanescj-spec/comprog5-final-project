...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <form method="GET" action="{{ route('admin.analytics.sales') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                <div>
                ...
                <div class="lg:col-span-2 flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('admin.analytics.sales') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-full hover:bg-gray-200" style="border-radius: 0.375rem !important;">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-lg font-semibold text-gray-900">Yearly Sales Bar Chart ({{ $selectedYear }})</h3>
                            ...
                            @foreach ($pieLabels as $index => $label)
                                ...
                                <li class="flex items-center justify-between border-b border-gray-100 pb-2">
                                    <span class="inline-flex items-center gap-2 text-gray-700 mr-3">
                                        <span class="product-color-dot h-3 w-3 rounded-full border border-gray-200" data-color="{{ $color }}"></span>
                                        {{ $label }}
                                    </span>
                                    <span class="flex flex-col items-end">
                                        <span class="text-gray-900 font-medium">{{ number_format($percent, 2) }}%</span>
                    ...
                    <table class="min-w-full text-sm" id="variant-breakdown-table">
                        ...
                        <tbody id="variant-breakdown-body">
                            @foreach ($topVariants as $variant)
                                <tr class="border-b last:border-0" data-units="{{ $variant['units_sold'] }}" data-sales="{{ $variant['total_sales'] }}">
                                    <td class="py-2 pr-4 text-gray-900">{{ $variant['product_name'] }}</td>
                                    <td class="py-2 pr-4 text-gray-700">{{ $variant['variant_name'] }}</td>
                                    <td class="py-2 pr-4 text-gray-700">{{ number_format($variant['units_sold']) }}</td>
                                    <td class="py-2 pr-4 text-gray-700">₱{{ number_format($variant['total_sales'], 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
...
@section('scripts')
<script id="sales-chart-data" type="application/json">{!! json_encode([
