...
<div class="flex items-center justify-between">
    ...
    <a href="{{ route('products.index') }}" class="text-sm text-blue-800 hover:text-blue-900">
        ← Back to Products
    </a>
</div>
@endsection
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid md:grid-cols-2 gap-8 p-8">
                ...
                <div>
                        ...
                        <div class="swiper-wrapper">
                            @foreach($allImages as $img)
                                <div class="swiper-slide flex items-center justify-center">
                                    <img src="{{ $img }}" alt="{{ $product->name }}" class="object-contain w-full h-full cursor-pointer rounded-xl" onclick="openImageModal(this.src)">
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="flex gap-2 mt-2 justify-center">
                        @foreach($allImages as $idx => $img)
                            <img src="{{ $img }}" alt="{{ $product->name }} thumbnail" class="w-16 h-16 object-cover rounded-lg border-2 border-gray-200 cursor-pointer hover:border-blue-500 transition" onclick="window.swiperMain.slideToLoop(<?= $idx ?>)">
                        @endforeach
                    </div>
                    @endif
                ...
                <div class="flex flex-col">
                    @if($product->categories->count())
                        <div class="mb-4 flex flex-wrap gap-2">
                            @foreach($product->categories as $cat)
                                <a href="{{ route('categories.products', $cat) }}"
                    ...
                    <div class="mb-6">
                        ...
                        <span class="text-3xl font-bold text-gray-900">₱{{ number_format($product->variants->min('price') ?? 0, 2) }}</span>
                    </div>
                    ...
                    @if($product->variants->count() > 0)
                        <div class="mb-6">
                            ...
                            <form method="POST" action="{{ route('cart.add') }}" id="add-to-cart-form" class="space-y-4">
                                ...
                                <div class="space-y-2">
                                    @foreach($product->variants as $variant)
                                        <label class="flex items-center justify-between p-3 border-2 rounded-lg cursor-pointer hover:border-blue-300 transition-colors variant-option {{ $variant->stock <= 0 ? 'opacity-50 cursor-not-allowed' : '' }}" data-variant-id="{{ $variant->id }}" data-stock="{{ $variant->stock }}" data-image="{{ $variant->image ? asset('storage/' . $variant->image) : '' }}">
                                            <input type="radio" name="variant_selection" value="{{ $variant->id }}" class="hidden" {{ $variant->stock <= 0 ? 'disabled' : '' }} onchange="selectVariant({{ $variant->id }})">
                                            <div class="flex items-center gap-3 flex-1" data-image="{{ $variant->image ? asset('storage/' . $variant->image) : '' }}" onclick="onVariantImageClick(event, this)">
                                                @if($variant->image)
                                                    <img src="{{ asset('storage/' . $variant->image) }}" alt="{{ $variant->variant_name }}" class="h-12 w-12 object-cover rounded cursor-pointer border-2 border-gray-200 hover:border-blue-500 transition" data-image="{{ asset('storage/' . $variant->image) }}" onclick="changeMainImageFromImage(this); event.stopPropagation();">
                                                @else
                                                ...
                                                @endif
                                                <div>
                                                    <div class="flex gap-2 items-center text-sm font-medium">{{ $variant->variant_name ?: 'Default Variant' }}</div>
                                                    <div class="text-xs text-gray-500 mt-1">
                                                        @if($variant->stock > 0)
                                                            <span class="text-emerald-600">{{ $variant->stock }} in stock</span>
                                                        @else
                                            ...
                                            <div class="text-lg font-bold text-gray-900">₱{{ number_format($variant->price, 2) }}</div>
                                        </label>
                                    @endforeach
                                </div>
                                ...
                                <div class="space-y-2">
                                    @foreach($product->variants as $variant)
                                        <div onclick="selectGuestVariant(this)"
                                            class="flex items-center justify-between p-3 border-2 rounded-lg transition-colors guest-variant {{ $variant->stock <= 0 ? 'opacity-50' : 'cursor-pointer hover:border-blue-300' }}"
                                            data-image="{{ $variant->image ? asset('storage/' . $variant->image) : '' }}">
                                            <div class="flex items-center gap-3">
                                                @if($variant->image)
                                                    <img src="{{ asset('storage/' . $variant->image) }}" alt="{{ $variant->variant_name }}" class="h-12 w-12 object-cover rounded border-2 border-gray-200 transition">
                                                @else
                                                    <div class="h-12 w-12 bg-gray-100 rounded flex items-center justify-center">
                                                        <svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                                ...
                                                @endif
                                                <div>
                                                    <div class="text-sm font-medium">{{ $variant->variant_name ?: 'Default Variant' }}</div>
                                                    <div class="text-xs text-gray-500 mt-1">
                                                        @if($variant->stock > 0)
                                                            <span class="text-emerald-600">{{ $variant->stock }} in stock</span>
                                                        @else
                    ...
                    <div class="mt-auto pt-6 flex gap-3 items-center">
                        @auth
                            ...
                            @if($product->variants->count() > 0)
                                ...
                                <button type="submit" form="add-to-cart-form" formaction="{{ route('orders.buyNow') }}" formmethod="POST" id="buy-now-btn" disabled class="flex-1 px-6 py-3 bg-gray-300 text-gray-500 font-semibold rounded-full cursor-not-allowed">Buy Now</button>
                                <form method="POST" action="{{ $isFavorited ? route('favorites.destroy', $product) : route('favorites.store') }}" class="ml-2">
                                    @csrf
                                    @if($isFavorited)
                                        @method('DELETE')
                            ...
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="flex-1 text-center px-6 py-3 bg-blue-700 text-white font-semibold rounded-full hover:bg-blue-800">Log in to Purchase</a>
                        @endauth
                    </div>
                </div>
            </div>
        ...
        <div class="mt-8 grid gap-6 md:grid-cols-3">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        @if($ratingsCount > 0)
                            <div class="text-sm text-gray-600">
                                ...
                                <span class="ml-1 text-xs text-gray-500">({{ $ratingsCount }} review{{ $ratingsCount > 1 ? 's' : '' }})</span>
                            </div>
                        @endif
                    </div>
                                ...
                                <div class="border border-gray-100 rounded-lg p-3">
                                    <div class="flex items-start gap-3 mb-2">
                                        @if($rating->user->profile_photo_path)
                                            <img src="{{ asset('storage/' . $rating->user->profile_photo_path) }}" alt="{{ $rating->user->name }}" class="h-10 w-10 rounded-full object-cover flex-shrink-0" style="border-radius: 50% !important;">
                                        @else
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            @if(!empty($reviewVariantByUser[$rating->user_id] ?? null))
                                                <p class="text-xs text-gray-500">Variant: {{ $reviewVariantByUser[$rating->user_id] }}</p>
                                            @endif
                                    ...
                                    <p class="text-sm text-gray-700 review-comment">{{ $rating->comment ?: 'No comment provided.' }}</p>
                                    @if($rating->photo_path)
                                        <div class="mt-2">
                                            <img src="{{ asset('storage/' . $rating->photo_path) }}" alt="Review photo" class="h-28 w-28 object-cover rounded-lg border border-gray-200">
                                        </div>
                                    @endif
                                </div>
                            @endforeach
            ...
            <div class="md:col-span-1 space-y-2">
                @auth
                    @if($userHasPurchased)
                        <form method="POST" action="{{ route('products.ratings.store', $product) }}" enctype="multipart/form-data" class="bg-blue-50 rounded-xl border border-blue-100 p-4 space-y-3">
                            @csrf
                            ...
                            <div>
                                ...
                                <div class="flex gap-1">
                                    @for($i = 1; $i <= 5; $i++)
                                        <input type="radio" name="rating" value="{{ $i }}" id="rating-{{ $i }}" class="hidden" required @checked((int) old('rating', $existingUserRating?->rating) === $i)>
                                        <label for="rating-{{ $i }}" class="text-2xl cursor-pointer text-gray-300 hover:text-amber-400 transition" data-value="{{ $i }}">☆</label>
                                    @endfor
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-2">Comment (optional)</label>
                                <textarea name="comment" maxlength="2000" rows="3" placeholder="Share your thoughts..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-blue-500">{{ old('comment', $existingUserRating?->comment) }}</textarea>
                                <p class="text-xs text-gray-500 mt-1"><span id="char-count">{{ strlen(old('comment', $existingUserRating?->comment ?? '')) }}</span>/2000</p>
                            </div>
                            <div>
                                ...
                                @if($existingUserRating?->photo_path)
                                    <p class="text-xs text-gray-500 mt-2 mb-1">Current uploaded photo</p>
                                    <div class="instant-image-preview-wrap relative mt-1 w-fit">
                                        <img src="{{ asset('storage/' . $existingUserRating->photo_path) }}" alt="Current review photo" class="instant-image-preview h-24 w-24 rounded-lg border border-gray-200 object-cover">
                            ...
                            <button type="submit" class="w-full px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition">
                                {{ $existingUserRating ? 'Update Review' : 'Submit Review' }}
                            </button>
                        </form>
                        <script>
                ...
                @else
                    <div class="text-center mt-2 mb-4">
                        ...
                        <a href="{{ route('login') }}" class="inline-block px-4 py-2 bg-blue-600 text-white text-xs font-medium rounded-full shadow hover:bg-blue-700 transition">Sign In</a>
                    </div>
                @endauth

                @foreach($product->categories as $cat)
                    <a href="{{ route('categories.products', $cat) }}" class="block w-full text-center px-4 py-3 bg-blue-50 text-blue-900 text-sm font-medium rounded-2xl hover:bg-blue-100 border border-blue-100">
                        View more in {{ $cat->name }} →
                    </a>
                @endforeach
            </div>
            @endif
